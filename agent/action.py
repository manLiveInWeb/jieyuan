...
class Status:
    ...
    def roi_image(self, image, roi):
        ...
        # cv2.imwrite("debug/coin_binary.png", binary_3ch)
    ...
    def update(self, context: Context):
        ...
        self.coin = int(self.ocr(context, coin_crop))
        self.ticket = int(self.ocr(context, ticket_crop))
        self.candlelight = int(self.ocr(context, candlelight_crop))
        self.show()
        self.init = True
...
def choice_way():
    if status.candlelight == 2:
        return "get_candlelight"
    if status.candlelight > 2 and status.coin >= 150:
